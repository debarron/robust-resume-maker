...
function getFirstName(langCallback) {
    ...
    getLanguagesFunc(ghUserID, langCallback)
...
function getLanguagesFunc(githubId,langCallback){
    var repAPIcall = "https://api.github.com/users/" + githubId+"/repos";
    var repRequest = new XMLHttpRequest();

    repRequest.addEventListener("load", reqLangListener(langCallback));
...
function reqLangListener(langCallback) {
    ...
    for(var i = 0; i < responseLang.length; i++){
        ...
        newReq.addEventListener( "load", getLanguages(langCallback));
